                         ...
                         curl_socket_t socket)
{
    ...
    printf("socket %d %d", socket, socket == NULL);
...
CURLOptionMapping implementedOptionMap[] =
{
  ...
  CURLOPT(RANGE),
  CURLOPT(ERRORBUFFER), /* mutable buffer, as output value, do not duplicate */
  CURLOPT(TIMEOUT),
  CURLOPT(POSTFIELDS),
  ...
  CURLOPT(USERAGENT),
  CURLOPT(FTPPORT),
  CURLOPT(LOW_SPEED_LIMIT),
  CURLOPT(LOW_SPEED_TIME),
  CURLOPT(RESUME_FROM),
  CURLOPT(COOKIE),
  CURLOPT(HTTPHEADER),
  CURLOPT(HTTPPOST),
  ...
  CURLOPT(SSLKEY),
  CURLOPT(SSLKEYTYPE),
  CURLOPT(SSLKEYPASSWD),
  CURLOPT(SSLENGINE),
  CURLOPT(SSLENGINE_DEFAULT),
  CURLOPT(CRLF),
  CURLOPT(QUOTE),
  CURLOPT(POSTQUOTE),
  ...
  CURLOPT(MAXCONNECTS),
  CURLOPT(CLOSEPOLICY),
  CURLOPT(FRESH_CONNECT),
  CURLOPT(FORBID_REUSE),
  CURLOPT(RANDOM_FILE),
  CURLOPT(EGDSOCKET),
  ...
  CURLOPT(COOKIEJAR),
  CURLOPT(SSL_CIPHER_LIST),
  CURLOPT(HTTP_VERSION),
  CURLOPT(FTP_USE_EPSV),
  CURLOPT(DNS_CACHE_TIMEOUT),
  CURLOPT(DNS_USE_GLOBAL_CACHE),
  CURLOPT(DEBUGFUNCTION),
  CURLOPT(PRIVATE),
#if HAVE_DECL_CURLOPT_HTTP200ALIASES
  CURLOPT(HTTP200ALIASES),
#else
  HAVENOT(HTTP200ALIASES),
#endif
#if HAVE_DECL_CURLOPT_UNRESTRICTED_AUTH
  CURLOPT(UNRESTRICTED_AUTH),
#else
  HAVENOT(UNRESTRICTED_AUTH),
#endif
#if HAVE_DECL_CURLOPT_FTP_USE_EPRT
  CURLOPT(FTP_USE_EPRT),
#else
...
#endif
#if HAVE_DECL_CURLOPT_FTP_CREATE_MISSING_DIRS
  CURLOPT(FTP_CREATE_MISSING_DIRS),
#else
...
#endif
#if HAVE_DECL_CURLOPT_INFILESIZE_LARGE
  CURLOPT(INFILESIZE_LARGE),
#else
  HAVENOT(INFILESIZE_LARGE),
#endif
#if HAVE_DECL_CURLOPT_RESUME_FROM_LARGE
  CURLOPT(RESUME_FROM_LARGE),
#else
  HAVENOT(RESUME_FROM_LARGE),
#endif
#if HAVE_DECL_CURLOPT_MAXFILESIZE_LARGE
  CURLOPT(MAXFILESIZE_LARGE),
#else
...
#endif
#if HAVE_DECL_CURLOPT_IGNORE_CONTENT_LENGTH
  CURLOPT(IGNORE_CONTENT_LENGTH),
#else
  HAVENOT(IGNORE_CONTENT_LENGTH),
#endif
#if HAVE_DECL_CURLOPT_FTP_SKIP_PASV_IP
  CURLOPT(FTP_SKIP_PASV_IP),
#else
  HAVENOT(FTP_SKIP_PASV_IP),
#endif
#if HAVE_DECL_CURLOPT_FTP_FILEMETHOD
  CURLOPT(FTP_FILEMETHOD),
#else
...
#endif
#if HAVE_DECL_CURLOPT_MAX_SEND_SPEED_LARGE
  CURLOPT(MAX_SEND_SPEED_LARGE),
#else
  HAVENOT(MAX_SEND_SPEED_LARGE),
#endif
#if HAVE_DECL_CURLOPT_MAX_RECV_SPEED_LARGE
  CURLOPT(MAX_RECV_SPEED_LARGE),
#else
  HAVENOT(MAX_RECV_SPEED_LARGE),
#endif
#if HAVE_DECL_CURLOPT_FTP_ALTERNATIVE_TO_USER
  CURLOPT(FTP_ALTERNATIVE_TO_USER),
#else
  HAVENOT(FTP_ALTERNATIVE_TO_USER),
#endif
#if HAVE_DECL_CURLOPT_SSL_SESSIONID_CACHE
  CURLOPT(SSL_SESSIONID_CACHE),
#else
  HAVENOT(SSL_SESSIONID_CACHE),
#endif
#if HAVE_DECL_CURLOPT_SSH_AUTH_TYPES
  CURLOPT(SSH_AUTH_TYPES),
#else
...
#endif
#if HAVE_DECL_CURLOPT_HTTP_TRANSFER_DECODING
  CURLOPT(HTTP_TRANSFER_DECODING),
#else
  HAVENOT(HTTP_TRANSFER_DECODING),
#endif
#if HAVE_DECL_CURLOPT_HTTP_CONTENT_DECODING
  CURLOPT(HTTP_CONTENT_DECODING),
#else
  HAVENOT(HTTP_CONTENT_DECODING),
#endif
#if HAVE_DECL_CURLOPT_NEW_FILE_PERMS
  CURLOPT(NEW_FILE_PERMS),
#else
  HAVENOT(NEW_FILE_PERMS),
#endif
#if HAVE_DECL_CURLOPT_NEW_DIRECTORY_PERMS
  CURLOPT(NEW_DIRECTORY_PERMS),
#else
...
#endif
#if HAVE_DECL_CURLOPT_SSH_HOST_PUBLIC_KEY_MD5
  CURLOPT(SSH_HOST_PUBLIC_KEY_MD5),
#else
